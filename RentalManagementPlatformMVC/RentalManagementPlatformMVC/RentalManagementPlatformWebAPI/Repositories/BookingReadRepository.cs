...
namespace RentalManagementPlatformAPI.Repository
{
	...
	{
		private readonly RentalManagementPlatformSqlContext _db;//使用EF Core的DbContext來操作資料庫
		public BookingReadRepository(RentalManagementPlatformSqlContext db) => _db = db;//建構函式注入：讓外部（例如 DI 容器）提供 DbContext 實例，支援測試與模組化。
		...
		{
			return await _db.Set<Booking>()//db.Set查找Booking資料表
		...
		public IQueryable<Booking> Query()//提供彈性查詢,讓service加入新的條件
		{
			return _db.Set<Booking>().AsQueryable();
		}
	}
}
