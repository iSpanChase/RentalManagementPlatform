    ...
    public class ReportFormController : Controller
    {
        ...
        List<decimal> BookingRevenueSum(DateTime[] intervals, int? cityId, int? districtId, string status)
        {
            using (var context = new RentalManagementPlatformSqlContext())
            {
                ...
                {
                    IQueryable<Models.Booking> bookingWhereAddress = BookingSelecter(context, intervals, i, cityId, districtId, status);
        ...
        private static IQueryable<Models.Booking> BookingSelecter
            (RentalManagementPlatformSqlContext context, DateTime[] intervals, int i, int? cityId, int? districtId, string status)
        {
            ...
            var result = context.Bookings
                .Where(x => x.CreatedAt != null &&
                            ...
                            x.TotalPrice != null)
                .Join(context.RoomLists, b => b.RoomId, rl => rl.RoomId, (b, rl) => new { b, rl })
                .Join(context.Addresses, brl => brl.rl.AddressId, a => a.AddressId, (brl, a) => new { brl, a })
                .Join(context.Districts, brla => brla.a.DistrictId, d => d.DistrictId, (brla, d) => new { brla, d })
                .Join(context.Cities, brlad => brlad.d.CityId, c => c.CityId, (brlad, c) => new { brlad, c })
